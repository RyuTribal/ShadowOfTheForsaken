...
namespace SOF
{
    ...
    namespace Utils
    {
        ...
        {
            if (multidimensional) { return multisampled ? GL_TEXTURE_2D_MULTISAMPLE_ARRAY : GL_TEXTURE_2D_ARRAY; }
          ...
          int array_size)
        {
            ...
            } else if (internalFormat == GL_RGBA16F || internalFormat == GL_RGB16F || internalFormat == GL_RGBA32F
            ...
            glFramebufferTexture2D(
              GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, TextureTarget(multisampled, multidimensional), id, 0);
        }
            ...
            if (multidimensional) {
                if (multisampled) {
                    glTexImage3DMultisample(
                      GL_TEXTURE_2D_MULTISAMPLE_ARRAY, samples, format, width, height, array_size, GL_FALSE);
                } else {
                ...
                glFramebufferTexture2D(
                  GL_FRAMEBUFFER, attachmentType, TextureTarget(multisampled, multidimensional), id, 0);
            }
        }
        ...
        static GLenum SOFDownSamplingFormatToGL(FramebufferSamplingFormat format)
        {
            ...
            switch (format) {
            case FramebufferSamplingFormat::Linear:
                return GL_LINEAR;
            case FramebufferSamplingFormat::Nearest:
                return GL_NEAREST;
            }
    ...
    void Framebuffer::Invalidate()
    {
        ...
        if (m_ColorAttachments.size() > 1) {
            SOF_ASSERT(m_ColorAttachments.size() <= 4);
            GLenum buffers[6] = {
        ...
        SOF_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
    ...
    void Framebuffer::Resize(uint32_t width, uint32_t height)
    {
        if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
            SOF_WARN("FrameBuffer", "Attempted to rezize framebuffer to {0}, {1}", width, height);
            return;
        }
    ...
    {
        SOF_ASSERT(attachmentIndex < m_ColorAttachments.size());
    ...
    void Framebuffer::SetTexture(uint32_t index, uint32_t texture_id, uint32_t mip_level)
    {
        ...
        SOF_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
          "Framebuffer incomplete after setting texture");
    }

    void Framebuffer::CopyFramebufferContent(std::shared_ptr<Framebuffer> other_buffer,
      FramebufferSamplingFormat sampling)
    {
