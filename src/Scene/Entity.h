...
namespace SOF {
	...
	public:
		...
		void AddComponent(Type component) {
			if (m_Scene != nullptr) {
				m_Scene->GetRegistry()->Add<Type>(m_Handle, component);
			}
		}
		...
		void RemoveComponent() {
			if (m_Scene != nullptr) {
				m_Scene->GetRegistry()->Remove<Type>(m_Handle);
			}
		}
		...
		bool HasComponent() {
			if (m_Scene != nullptr) {
				return m_Scene->GetRegistry()->Get<Type>(m_Handle) != nullptr ? true : false;
			}
			return false;
		}
