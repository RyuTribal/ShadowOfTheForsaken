...
class Registry
{
  public:
    template<typename T> void Add(UUID entityId, T component)
    {
        ...
        std::static_pointer_cast<ComponentContainer<T>>(m_Components[typeIndex])->Add(entityId, std::move(component));
    }
    ...
    template<typename T> void Remove(UUID entityId)
    {
        const std::type_index typeIndex = std::type_index(typeid(T));
        auto it = m_Components.find(typeIndex);
        if (it != m_Components.end()) { std::static_pointer_cast<ComponentContainer<T>>(it->second)->Remove(entityId); }
    }
