...
namespace SOF {
	...
	void AssetManager::DeregisterAsset(std::string& asset_handle)
	{
		...
		if (s_Data->Manager->m_Assets[asset_handle].use_count() > 1) {
			SOF_ERROR("AssetManager", "Asset {0} is currently in use, please remove it first from the scene before deregistering", asset_handle);
			return;
		}
	...
	void AssetManager::WriteTOC(uint64_t deleted_offset, uint64_t deleted_length)
	{
		...
		while (!temp_file.eof()) {
			...
			if (deleted_offset > 0 && deleted_length > 0 &&
				current_pos >= std::streampos(deleted_offset) && current_pos < std::streampos(deleted_offset + deleted_length)) {
	...
	void AssetManager::AssetPackInit(std::filesystem::path path_to_assetpack)
	{
		...
		SOF_ASSERT(s_Data->Manager->m_OutputFile.gcount() == sizeof(GlobalHeader), "Failed to read the global header from file: {0}", path_to_assetpack.string());


		SOF_ASSERT(std::strncmp(m_GlobalHeader.Signature, "SOFP", 4) == 0, "Invalid file format: {0}", path_to_assetpack.string());
