...
namespace SOF {
    ...
    public:
        ...
        template <typename T>
        static std::shared_ptr<T> Load(std::string& asset_handle) {
            ...
            std::shared_ptr<Asset> asset = Instance()->m_Deserializers[static_cast<AssetType>(toc_entry.Type)]->Load(toc_entry, buffer);

            if (asset != nullptr) {
                Instance()->m_Assets[asset_handle] = asset;
            }
